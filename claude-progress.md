...
- `CREW-008` is **in progress**: GitHub Actions workflows added for static baseline (`./init.sh`) and GHCR Docker publish; awaiting first Actions run on push/merge to `main` for PR build and GHCR publish evidence.
...
- 2026-06-13 `CREW-007` added the dry-run-only FastAPI service: `tests.test_api` passed (3 cases: healthz, /run handoff, 400 out-of-workspace); `fastapi==0.136.3` + `uvicorn[standard]` added and `uv.lock` refreshed; `./init.sh` passed (31 tests) with `Dockerfile` as a required file. The image was built with `podman` and verified in-container (healthz + /run over the read-only workspace mount); the Dockerfile installs `git` (needed by the repo-state adapter) and `run_command` was hardened to degrade (returncode 127) when a binary is missing. Host→container port-forwarding was flaky under rootless podman in WSL (environment quirk), so the container was verified from inside.
- 2026-06-13 `CREW-008` added GitHub Actions: `.github/workflows/static.yml` and `docker-publish.yml`; `docker-compose.yml` tag updated to `latest`; README CI section added; `./init.sh` passed (31 tests). GHCR publish evidence pending first Actions run after push to `main`.
...
- `CREW-008`: first GHCR publish requires push/merge to `main` and org/repo Actions package write permission; set GHCR package visibility after first publish.

## Recommended Next Step

Push/merge CREW-008 to `main`, confirm both GitHub Actions workflows pass, then mark `CREW-008` passing with the Actions run URL and `docker pull ghcr.io/d3hl/d3hl_infra_crew:latest`. Use `docker compose up` with the published image for the dry-run-only API.
